...
class EngineSpeedElement extends HTMLElement {
  ...
  #bindToStore(s?: { speed: number }) {
    ...
    // scheduler: coalesce to rAF (one update per frame)
    let scheduled = false;
    ...
    this.mobxDispose = reaction(
      ...
      (v) => {
        ...
        requestAnimationFrame(() => {
          scheduled = false;
          // fast path: update internal state/angle only
          this.#setTargetFromSpeed(nextVal);
          // (no attribute reflection here)
        });
      },
      {
        fireImmediately: true, // set initial speed right away
        equals: (a, b) => a === b, // primitives: strict equality is perfect
      }
    );
  }
  ...
  #updateNeedle(angleRad: number) {
    ...
    const cy = h / 2;
    const radius = Math.min(w, h) * 0.49;
    // const width = Math.max(3, radius * 0.02);
  ...
  #updateProgressArc(currentAngleRad: number) {
    ...
    // nice, slightly inside the main arc so it doesn't overlap edges
    const trackRadius = radius * 0.95;
    const trackWidth = Math.max(4, radius * 0.08);

    // draw only if there's span
    if (cur > a0 + 1e-6) {
      this.progressArc
